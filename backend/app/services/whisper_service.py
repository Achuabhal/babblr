...
class WhisperService(STTService):
    ...
    def __init__(self, model_size: str = "base", device: str = "auto"):
        ...
        try:
            logger.info(
                "Loading Whisper model: %s on device: %s", model_size, self.device
            )
            ...
            logger.info(
                "Whisper model loaded successfully in %.2f seconds", load_time
            )
    ...
    async def transcribe(
        ...
        try:
            ...
            result = await asyncio.wait_for(
                self._transcribe_sync(converted_path, language),
                timeout=timeout
            )
    ...
    async def _convert_audio_format(self, audio_path: str) -> str:
        ...
        if not PYDUB_AVAILABLE:
            logger.warning(
                "pydub not available, cannot convert %s format. "
        ...
        try:
            ...
            logger.warning(
                "Failed to convert audio format: %s. Using original file.", str(e)
            )
            return audio_path
