...
async def transcribe_audio(
    ...
    try:
        ...
        # Save file in development mode
        if settings.development_mode:
            saved_file_path = await _save_audio_file(temp_file.name, audio_file.filename)
...
async def get_supported_languages():
    ...
    return JSONResponse(
        content={
            "languages": [
                language_details.get(code, {"code": code, "name": code})
...
async def get_available_models():
    ...
    return JSONResponse(
        content={
            "models": [
                model_details.get(model, {"name": model})
                for model in models
            ],
